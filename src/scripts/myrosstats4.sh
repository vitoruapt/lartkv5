...
statFile=/tmp/lar4stats.txt
HTMLout=/tmp/lar4stats.html
...
#for lar4
pkgFile='package.xml'
homeLarScripts="$HOME/lar4/src/scripts"
LARDOC="$HOME/lar4/doc"
...
if [ $unattended = 1 -o $ForceEmail = 1 ]; then
	...
	outputResult=/tmp/lar4stats.out
...
if [ -d "$dir" ]; then
	...
	#For lar4
	doxygenW="$HOME/lar4/doc/alldoxygen.warnings"
...
pageTitle="Statistics of LAR Toolkit V4"
tableCaption='Statistics of LAR Toolkit V4 R'$svnRevision
...
if [ $unattended = 1 -o $ForceEmail = 1 ]; then
		mail -s "[LARtk] LAR4 rev. $svnRevision statistics" -a "From: LAR Toolkit Watcher <root@lars.mec.ua.pt>" -a "Content-Type: text/html; charset=\"UTF-8\"" -t $userList < $HTMLout
