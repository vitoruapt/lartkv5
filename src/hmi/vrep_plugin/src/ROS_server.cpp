...
std::string ROS_server::addPublisher(const char* topicName,int queueSize,int streamCmd,int auxInt1,int auxInt2,const char* auxString,int publishCnt)
{
	...
	{ // We launched the publisher!
		...
		std::ofstream myfile;
		myfile.open("/home/phua-lar/out_add.txt", std::ios::app);
		myfile << "Advertising topic: " << pub.topicName << "\n";
		myfile.close();
...
bool ROS_server::launchPublisher(SPublisherData& pub,int queueSize)
{
	...
	if (pub.cmdID==simros_strmcmd_get_imu1_data || pub.cmdID==simros_strmcmd_get_imu2_data || pub.cmdID==simros_strmcmd_get_imu3_data || pub.cmdID==simros_strmcmd_get_imu4_data ||
	    pub.cmdID==simros_strmcmd_get_imu5_data || pub.cmdID==simros_strmcmd_get_imu6_data || pub.cmdID==simros_strmcmd_get_imu7_data || pub.cmdID==simros_strmcmd_get_imu8_data ||
	    pub.cmdID==simros_strmcmd_get_imu9_data)
	{
		pub.generalPublisher=node->advertise<sensor_msgs::Imu>(pub.topicName,queueSize); /* Added by João O. Barros on 22/12/2014; */
		
// 		std::ofstream myfile;
// 		myfile.open("/home/phua-lar/out_strmcmd.txt", std::ios::app);
// 		myfile << "Advertising topic: " << pub.topicName << "\n";
// 		myfile.close();
...
void ROS_server::streamAllData()
{
	...
	for (int pubI=0;pubI<int(publishers.size());pubI++)
	{
		...
		{ // we handle only the publishers that are not asleep!
			...
			if (publishers[pubI].cmdID==simros_strmcmd_get_imu1_data || publishers[pubI].cmdID==simros_strmcmd_get_imu2_data || publishers[pubI].cmdID==simros_strmcmd_get_imu3_data || publishers[pubI].cmdID==simros_strmcmd_get_imu4_data ||
			    publishers[pubI].cmdID==simros_strmcmd_get_imu5_data || publishers[pubI].cmdID==simros_strmcmd_get_imu6_data || publishers[pubI].cmdID==simros_strmcmd_get_imu7_data || publishers[pubI].cmdID==simros_strmcmd_get_imu8_data ||
			    publishers[pubI].cmdID==simros_strmcmd_get_imu9_data)
			{      
				float val[6];

				std::string stringSignalName=publishers[pubI].auxStr;
				int signalLength;
				const char* signalValue=simGetStringSignal(stringSignalName.c_str(),&signalLength);
...
// 				    simReleaseBuffer(signalValue);
				    ...
				    fl.header.seq=_simulationFrameID;
				    fl.header.stamp=inf.headerInfo.stamp;
				    fl.header.frame_id=stringSignalName[3];
